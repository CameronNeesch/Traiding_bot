...
q=2*p*0.9
order_size=round(float(q/get_symbol_price(symbol)),3)
...
percent1 = int(get_symbol_price(symbol)*0.013)
percent2 = int(get_symbol_price(symbol)*0.018)
percent3 = int(get_symbol_price(symbol)*0.023)
percent4 = int(get_symbol_price(symbol)*0.028)


# In[5]:


maxposition=order_size
stop_percent=0.015 # 0.01=1%
eth_proffit_array=[[percent1,1],[percent2,3],[percent3,3],[percent4,3],[200,0]]
...
telegram_delay=12
bot_token='2140904231:AAEQhSj70rCmU7Ge9UCXde6Zy0GCK3i-a6I'
chat_id='279611347'
...
<ipython-input-22-ad245339b4db> in <module>
      5 while time.time() <= timeout:
      6     try:
----> 7         prt("script continue running at "+time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time())))
      8         main(counterr)
      9         counterr=counterr+1

<ipython-input-20-79bbda3435b5> in prt(message)
      2     # telegram message
      3     telegram_bot_sendtext(pointer+': '+message)
----> 4     print(pointer+': '+message)
      5 
